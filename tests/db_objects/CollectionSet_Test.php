...
{
	protected static ?string $workerClass = "YAPF\Junk\BrokenDbObjectMassive";
}
...
class CollectionSetTest extends TestCase
{
	...
	{
		$result = CounttoonehundoSet::blindUpdate("cvalue","123",null);
		$this->assertSame(true,$result->status,"blind update has failed with error: ".$result->message);
		$this->assertSame(100,$result->changes,"Incorrect number of changes expected 100 but got: ".$result->changes);
		...
		$result = CounttoonehundoSet::blindUpdate("cvalue","444",$whereConfig);
		$this->assertSame(true,$result->status,"blind update has failed with error: ".$result->message);
		$this->assertSame(4,$result->changes,"Incorrect number of changes expected 4 but got: ".$result->changes);
	}

	public function testBlindRemove()
	{
		$loop = 0;
		while($loop < 100)
		{
			$newObj = new Counttoonehundo();
			$newObj->_Cvalue = 100 + 1 + $loop;
			$newObj->createEntry();
			$loop++;
		}
        $whereConfig = [
		...
		$result = CounttoonehundoSet::blindRemove(where: $whereConfig);
		$this->assertSame(true,$result->status,"blind update has failed with error: ".$result->message);
		$this->assertSame(100,$result->itemsRemoved,"Incorrect number of removals expected 100 but got: ".$result->itemsRemoved);
	}
}
