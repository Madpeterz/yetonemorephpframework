...
class SingleModelFactory extends ModelFactoryShared
{
    ...
    {
        // make sure the table has a "id" field, or fail
        $found = false;
        ...
        if($found == false)
        {
            throw new Exception("id field is required skipping");
        }
        $this->fileLines[] = [0];
        $this->fileLines[] = '}';
    ...
    public function createModelGetSet(): void
    {
        ...
        foreach ($this->cols as $rowTwo) {
            ...
            $this->fileLines[] = "public ".$nullable."".$useType." $".$underscoreValue."".ucfirst($rowTwo["COLUMN_NAME"])." {";
            $this->fileLines[] = [2];
            $this->fileLines[] = "get => \$this->getField(fieldName: \"".$rowTwo["COLUMN_NAME"]."\");";
            $this->fileLines[] = "set {";
            $this->fileLines[] = [3];
            $this->fileLines[] = "\$this->updateField(fieldName: \"".$rowTwo["COLUMN_NAME"]."\", value: \$value);";
            $this->fileLines[] = [2];
            $this->fileLines[] = "}";
            $this->fileLines[] = [1];
            $this->fileLines[] = "}";
    ...
    protected function createModelSetters(): void
    {
        ...
        foreach ($this->cols as $rowTwo) {
            ...
            $this->fileLines[] = "*/";
            $set_function = 'protected function set' . ucfirst($rowTwo["COLUMN_NAME"]);
            $set_function .= '(' . $returnType . ' $newValue): void';
            $this->fileLines[] = $set_function;
            $this->fileLines[] = '{';
            $this->fileLines[] = [2];
            $this->fileLines[] = '$this->updateField("' . $rowTwo["COLUMN_NAME"] . '", $newValue);';
            $this->fileLines[] = [1];
            $this->fileLines[] = '}';
        }
    }
    ...
    protected function createModelGetters(): void
    {
        ...
        foreach ($this->cols as $rowTwo) {
            if ($rowTwo["COLUMN_NAME"] != "id") {
                ...
                $returnType = ": ?" . $useType . "";
                $getFunction = 'protected function get' . ucfirst($rowTwo["COLUMN_NAME"]);
