                ...
                if ($failed == false) {
                    if ($sql != "empty_in_array") {
                        if (is_array($order_config) == true) {
                            $this->build_select_orderby($sql, $order_config, $main_table_id, $auto_ids);
                        }
                        if (is_array($options_config) == true) {
                            $this->build_select_option($sql, $options_config);
                        }
                        $this->lastSql = $sql;
                        if ($stmt = $this->sqlConnection->prepare($sql)) {
                            ...
                            if ($bind_ok == true) {
                                if ($stmt->execute() == true) {
                                    ...
                                    return ["status" => false,"message" => "Unable to execute"];
                                }
                            } else {
                                return ["status" => false,"message" => "Unable to bind: " . $bind_text . " " . print_r($bind_args, true) . ""];
                            }
                        } else {
                            return ["status" => false,"message" => "Unable to prepair: " . $sql];
                        }
                    } else {
                        return ["status" => true,"message" => "no data empty in array.","dataSet" => []];
                    }
                } else {
                    return ["status" => false,"message" => "failed with message:" . $failed_on];
                }
            } else {
                return ["status" => false,"message" => "Unable to connect to database"];
            }
        } else {
            return ["status" => false,"message" => "table is required in basic_config arg"];
        }
    }
}
