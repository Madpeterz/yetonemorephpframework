                ...
                if (count($update_config["fields"]) == count($update_config["values"])) {
                    if (count($update_config["types"]) == count($update_config["values"])) {
                        if (count($update_config["types"]) > 0) {
                            ...
                            $failed_on = "Update types must have a count of 1 or higher!";
                        }
                    } else {
                                    $failed_on = "Update types does not match update values count!";
                    }
                } else {
                    $failed_on = "Update fields does not match update values count!";
                }
                            ...
                            if ($stmt = $this->sqlConnection->prepare($sql)) {
                                ...
                                if ($bind_ok == true) {
                                    if ($stmt->execute() == true) {
                                        ...
                                        return $this->failure("Failed to bind");
                                }
                            } else {
                                return $this->failure("Failed to prepair");
                            }
                        } else {
                            return $this->failure("Targeting IN|NOT IN with no array");
                        }
                    } else {
                        return $this->failure("Failed: " . $failed_on . "");
                    }
                } else {
                    return $this->failure("Failed: " . $failed_on . "");
                }
            } else {
                return $this->failure("No table selected for update");
            }
        } else {
            return $this->failure("Unable to start SQL");
        }
    }
}
