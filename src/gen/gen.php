...
function get_col_type(string $target_type,string $col_type,string $table="",string $colname="",bool $add_warnings=false) : string
{
    global $warnings;
    $detected_type = "str";
    $string_types = array("varchar","text","char","longtext","mediumtext","tinytext","date","datetime");
    $int_types = array("int","smallint","bigint","mediumint","enum","timestamp");
    $float_types = array("decimal","float","double");
    if(in_array($target_type,$int_types)) $detected_type = "int";
    else if(in_array($target_type,$string_types)) $detected_type = "str";
    else if(in_array($target_type,$float_types)) $detected_type = "float";
    else if($target_type == "tinyint")
    {
    ...
    else $warnings[] = "Table: ".$table." Column: ".$colname." unknown type: ".$target_type." defaulting to string!<br/>";
    return $detected_type;
}

$tab_lookup = array(0 => "",1 => "\t",2 => "\t\t",3 => "\t\t\t"); // change to space if you want you monster
foreach(source_databases as $gen_database_name)
{
    ...
    $wherefields = array(array("TABLE_SCHEMA" => "="));
    $wherevalues = array(array($gen_database_name => "s"));
    ...
    if($data["status"] == true)
    {
        foreach($data["dataSet"] as $row)
        {
            ...
            $fields = array("COLUMN_NAME","COLUMN_DEFAULT","DATA_TYPE","COLUMN_TYPE");
            $wherefields = array(array("TABLE_SCHEMA" => "="),array("TABLE_NAME" => "="));
            $wherevalues = array(array($gen_database_name => "s"),array($row["TABLE_NAME"] => "s"));
            ...
            if($data_two["status"] == true)
            {
                ...
                */
                ...
                foreach($data_two["dataSet"] as $row_two)
                {
                    ...
                    $detected_default = "";
                    if(($row_two["COLUMN_DEFAULT"] == null) || ($row_two["COLUMN_DEFAULT"] == "NULL")) $detected_default = "null";
                    else $detected_default = $row_two["COLUMN_DEFAULT"];
                    if($use_type == "str")
                    {
                ...
                foreach($data_two["dataSet"] as $row_two)
                {
                    if($row_two["COLUMN_NAME"] != "id")
                    {
                        ...
                        if($use_type == "float") $use_type = "double";
                        else if($use_type == "str") $use_type = "string";
                ...
                $file_lines[] = array(0);
                $file_lines[] = '?>';
                if(file_exists($create_file)) unlink($create_file);
                $file_content = "";
                $tabs = 0;
            ...
            else echo "<font color=\"#FF0000\">ERROR</font> ".$data_two["message"]."<br/>";
        }
    }
}
?>
